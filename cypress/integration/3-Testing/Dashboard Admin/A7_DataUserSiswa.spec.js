describe("Login Admin", function () {
    ...
    it("Tambah Data User Siswa", function () {
        ...
        cy.get('#email').type('zafan@gmail.com')
        cy.get('button')
        cy.get('.col-md-12 > :nth-child(2)')
        cy.get('#select2-role-container').click().get('#role').select('Siswa', { force: true })
        cy.get('button')
        cy.get('#nomer').type('7890')
        cy.get('#password').type('zafan123')
        cy.get('#password-confirm').type('zafan123')
        ...
        cy.contains("Berhasil menambahkan user Siswa baru!", { timeout: 10000 }).should('be.visible')
        ...
        cy.url().should('include', '/login');
        cy.get("#email").type('zafan@gmail.com');
        cy.get("#password", { timeout: 10000 }).type('zafan123');
        cy.get("#btn-login").click();
        cy.contains("Dashboard").should("be.visible");
